...
export default {
  async fetch(request) {
    try {
      ...
      if (!payload.amount) return json(400, { error:'Amount is required.' });
      if (!payload.email) return json(400, { error:'Email is required.' });
      if (!payload.phone) return json(400, { error:'Phone number is required.' });
      const data = await submitOrder(request, payload);
      return json(200, data);
    } catch (err) {
      console.error('Pesapal payment request error:', err);
      return json(500, { error:err.message || 'Pesapal payment request failed.', details:err.data || null });
    }
  }
};
