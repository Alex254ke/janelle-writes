  ...
  async fetch(request) {
    try {
      ...
      if (adminSecret) {
        ...
        const provided = request.headers.get('x-admin-secret') || u.searchParams.get('admin_secret');
        if (provided !== adminSecret) return json(401, { error:'Unauthorized' });
      }
      const bearer = await token();
      const ipn = await ensureIpn(request, bearer);
      return json(200, { ok:true, ...ipn, message: ipn.reused ? 'Existing PESAPAL_IPN_ID reused.' : 'IPN registered. Copy ipn_id into Vercel as PESAPAL_IPN_ID and redeploy.' });
    } catch (err) {
      console.error('Pesapal register IPN error:', err);
      return json(500, { error:err.message || 'Could not register IPN.', details:err.data || null });
    }
  }
};
